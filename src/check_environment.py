...
def run_checks(raise_exc=False):
    ...
    if current_version >= Version('3.10.0') and current_version <= Version('3.10.14'):
        print(OK, 'Python is version %s\n' % sys.version)
    else:
        print(FAIL, 'Python version >= 3.10.0 and <= 3.10.14 is required, but %s is installed.\n' % sys.version)
        ...
        try:
            ...
            if req_version:
                ...
                if isinstance(req_version, list):
                    ...
                    if Version(version) < Version(min_version) or Version(version) > Version(max_version):
                        print(FAIL, '%s version >= %s and <= %s is required, but %s installed.' % (
                            pkg, min_version, max_version, version))
                        failures.append(pkg)
                        continue
                else:
